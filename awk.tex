...
\begin{enumerate}
  ...
  \item We change the height field with the following command: ``\$2=185cm'':\\
...
This would all be done at once, writing the output to a new file \textit{heights\_corrected.txt}, as follows:
...
\textcolor{gray}{$>$} \textcolor{yellow}{print} \textcolor{red}{\$1}``\textcolor{red}{\textbackslash t\textbackslash t}''a[0]\}' $>$ heights\_without\_units.txt
\end{tabular}
}}
\normalfont
...
There's a whole lot more to AWK than this -- the most important elements probably being the 'BEGIN' \& 'END' blocks and AWK's ability to use variables -- but that's all for now. Enjoy!
